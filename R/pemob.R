...
error_inds <- sapply(iter_pemob, \(x) nrow(unique(x[, 2])) != 6)
...
all_years <- as.character(2019:2024)

indicators <- map(all_years, \(x) {
  as.numeric(str_detect(tbl_errors$missing, as.character(x)))
})
...
control |>
  mutate(
    flag = ifelse(
      status_variable != status_variable_pemob | is.na(status_variable_pemob),
      1L,
      0L
    )
  ) |>
...
control |>
  mutate(
    ...
    status = case_when(
      status_variable == "000010" ~ "Variável não consta na PEMOB 2020",
      status_variable == "000001" ~ "Variável não consta na PEMOB 2019"
    )
  ) |>
  filter(flag == 1) |>
  show_in_excel()
